...
- 用途：比赛 Demo 中，前端提交任意合法 WGS84 经纬度，后端始终以东京都厅固定坐标 `35.6896342, 139.6917418` 为查询原点，返回该原点 3 公里内按直线距离排序的避难所候选。
...
`latitude` 与 `longitude` 必填，须为合法 WGS84 有限数值，以保持现有前端 Contract 兼容；Demo 服务不会使用这两个请求值进行距离、过滤或排序。`limit` 可省略，默认 5，范围为 1–10 的整数。响应 `origin` 始终为东京都厅固定坐标。
...
距离始终从东京都厅固定坐标出发，采用 Haversine 直线距离并四舍五入为整数米；请求坐标不会改变候选、距离或顺序。Google Maps URL 只定位设施坐标，不表示路线安全或可通行。设施数据不是实时开放信息。
